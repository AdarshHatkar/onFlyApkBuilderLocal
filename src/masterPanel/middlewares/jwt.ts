...
import { masterAccessTokenPayloadType } from "../helpers/jwtHelper.js";
...
export const verifyMasterAccessTokenMiddleware = trpcMiddleware(async ({ ctx, next }) => {
    try {
        ...
        let payload = await jwtVerifyAsync(token, process.env.ACCESS_TOKEN_SECRET) as masterAccessTokenPayloadType
